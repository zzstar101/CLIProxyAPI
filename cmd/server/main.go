...
func main() {
	...
	flag.BoolVar(&opencodeGoLogin, "opencode-go-login", false, "Login to OpenCode Go using OAuth")
	...
	commandMode := vertexImport != "" || antigravityLogin || codexLogin || codexDeviceLogin || claudeLogin || kimiLogin || xaiLogin || opencodeGoLogin
	...
	} else if opencodeGoLogin {
		cmd.DoOpenCodeGoLogin(cfg, options)
	} else {
